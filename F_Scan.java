...
public class F_Scan {
    public static void main(String[] args) {
        ...
        fScan(requests, headPosition, totalCylinder);
    }

    private static void fScan(List<Integer> requests, int headPosition, int totalCylinder) {
        ...
        int lastRequest = sortedRequests.get(sortedRequests.size() - 1);
        if (lastRequest != totalCylinder - 1) {
            System.out.println("Processing request: " + totalCylinder);
            totalSeekOperations += Math.abs(totalCylinder - currentHeadPosition);
            currentHeadPosition = totalCylinder;
        }
